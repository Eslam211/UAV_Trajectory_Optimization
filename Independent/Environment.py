...
class environment():
    ...
    def __init__(self, Dev_Coord,config):
        ...
        a1 = list(range(0,self.C+1)) # select one of the devices or select none.
        a3 = list(range(0,self.directions)) # move up, down, right, left or don't move.
        a5 = list([a1,a3]) # concatenate both actions
        self.all_actions = list(itertools.product(*a5)) # generate a lookup table
        self.action_space = np.arange((self.C+1)*self.directions) # size of the action space
    ...
    def step(self,state,Action_all):
        ...
        reward = [0]*self.U # initialized rewards
